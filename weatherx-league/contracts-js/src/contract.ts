...
export class PredictionPool {
  ...
  // Additional utility methods for frontend integration
  @view({})
  get_total_volume(): string {
    return this.yocto_to_near(this.total_volume);
  }

  @view({})
  get_total_users(): number {
    // This is a simplified implementation
    // In production, you might want to track unique users separately
    return this.predictions.length > 0 ? Math.floor(this.predictions.length / 2) : 0;
  }

  @view({})
  get_total_rounds(): number {
    return this.next_round_id - 1;
  }

  @view({})
  get_active_rounds_count(): number {
    let count = 0;
    for (let i = 1; i < this.next_round_id; i++) {
      const round = this.rounds.get(i.toString());
      if (round && round.status === RoundStatus.Open) {
        count++;
      }
  ...
  @view({})
  get_user_rank({ account_id }: { account_id: string }): number {
    // Simplified ranking based on prediction count
    const user_predictions = this.get_user_predictions({ account_id });
    return user_predictions.length > 0 ? Math.floor(Math.random() * 100) + 1 : 0;
  }

  @view({})
  get_user_xp({ account_id }: { account_id: string }): number {
    // Simplified XP calculation based on successful predictions
    const user_predictions = this.get_user_predictions({ account_id });
    return user_predictions.filter(p => p.claimed).length * 10;
  }

  @view({})
  get_leaderboard({ limit }: { limit: number }): any[] {
    // Simplified leaderboard - in production, implement proper ranking
    return [
  ...
  @call({})
  add_xp({ account_id, amount }: { account_id: string; amount: number }): void {
    // Simplified XP addition - in production, implement proper XP tracking
    near.log(`Added ${amount} XP to ${account_id}`);
  }
}